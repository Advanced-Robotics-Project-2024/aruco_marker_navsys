from setuptools import find_packages, setup
from glob import glob

package_name = 'aruco_marker_detector_py'
submodule=package_name+'/submodules'

setup(
    name=package_name,
    version='0.0.0',
    # packages=find_packages(exclude=['test']), 
    packages=[package_name, submodule], 
    # packages=[package_name], 
    data_files=[
        ('share/ament_index/resource_index/packages',
            ['resource/' + package_name]),
        ('share/' + package_name, ['package.xml']),
        ('share/' + package_name, glob('camera/*.*')) 
    ],
    install_requires=['setuptools'],
    zip_safe=True,
    maintainer='makotoyoshigoe',
    maintainer_email='myoshigo0127@gmail.com',
    description='TODO: Package description',
    license='Apache-2.0',
    tests_require=['pytest'],
    entry_points={
        'console_scripts': [
            'aruco_marker_info_pub = aruco_marker_detector_py.aruco_marker_info_pub:main', 
            'approach_marker_client = aruco_marker_detector_py.test_action_client:main'
        ],
    },
)
